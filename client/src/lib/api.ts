...
import { AuthData, PostVisibility, FriendRequestPrivacy, StoryElement, UserProfile } from './types';
...
export const translateText = async (text: string, targetLanguage: string): Promise<{ translatedText: string; detectedSourceLanguage: string; sourceType: 'human' | 'animal' | 'dialect'; confidence: number }> => {
  try {
    const response = await api.post('/universal-translate', { text, targetLanguage });
...
export const createPost = async (
  ...
  const mediaData = await Promise.all(
    media.map(async (mediaFile) => {
      ...
      if (mediaFile.captionsFile) {
        captionsUrl = await uploadPostMedia(mediaFile.captionsFile);
      }
      return {
        url,
        type: mediaFile.file.type.startsWith('video') ? 'video' : mediaFile.file.type.startsWith('audio') ? 'audio' : 'image',
  ...
  const response = await api.post('/posts', {
    ...
    autoTags, // Pass AI-generated auto-tags to the backend
    isSensitive,
...
export const searchUsers = async (query: string): Promise<CompetitorProfile[]> => {
