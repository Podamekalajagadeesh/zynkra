...
export const ThoughtEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const captureInterval = window.setInterval(() => {
      ...
      setRawThoughts(prev => [newThought, ...prev].slice(0, 50));
      addToast({ type: 'info', message: 'New thought captured!' });
    }, 5000);

    return () => window.clearInterval(captureInterval);
  }, [isCapturing, neuralState, addToast]);
  ...
  const publishThought = async () => {
    ...
    try {
      ...
      addToast({ type: 'success', message: 'Your thought has been shared successfully!' });
      setRawThoughts(prev => prev.filter(t => t.id !== selectedThought.id));
      setSelectedThought(null);
      setEditedThought(null);
      setActiveTab('capture');
    } catch (error) {
      addToast({ type: 'error', message: 'Failed to share your thought. Please try again.' });
    } finally {
  ...
  const deleteThought = (thoughtId: string) => {
    ...
    addToast({ type: 'success', message: 'Thought deleted' });
  };
  ...
  const duplicateThought = (thought: RawThought) => {
    ...
    addToast({ type: 'success', message: 'Thought duplicated for editing' });
  };
  ...
  const handleAIPolishedContent = (polishedContent: any) => {
    if (editedThought && selectedThought) {
      ...
      addToast({ type: 'success', message: 'AI refinements applied to your thought!' });
    }
  };
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'capture' | 'edit')} className="w-full">
          ...
          <Card className="p-6">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Capture your thoughts in real time and refine them before sharing.
                </p>
              </div>
              <Button onClick={isCapturing ? stopCapture : startCapture} variant={isCapturing ? 'destructive' : 'default'} className="shrink-0">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button variant={editedThought.visibility === 'only_me' ? 'default' : 'ghost'} size="sm" onClick={() => setEditedThought({ ...editedThought, visibility: 'only_me' })}>
                        <Lock className="h-4 w-4 mr-1" />
                        Only Me
                      </Button>
                      <Button variant={editedThought.visibility === 'friends' ? 'default' : 'ghost'} size="sm" onClick={() => setEditedThought({ ...editedThought, visibility: 'friends' })}>
                        <Users className="h-4 w-4 mr-1" />
                        Friends
                      </Button>
                      <Button variant={editedThought.visibility === 'public' ? 'default' : 'ghost'} size="sm" onClick={() => setEditedThought({ ...editedThought, visibility: 'public' })}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch checked={editedThought.allowComments} onCheckedChange={(checked) => setEditedThought({ ...editedThought, allowComments: checked })} />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch checked={editedThought.addContext} onCheckedChange={(checked) => setEditedThought({ ...editedThought, addContext: checked })} />
                  </div>
                </div>
              </Card>

              <div className="flex justify-end gap-4">
                <Button variant="ghost" onClick={() => { setSelectedThought(null); setEditedThought(null); setActiveTab('capture'); }}>
                  Cancel
                </Button>
