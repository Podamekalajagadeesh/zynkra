...
export function PostCard({
  ...
  return (
    <div className="card-hover mb-lg p-5 animate-fade-in sm:p-6" role="article" aria-labelledby={`post-content-${post.id}`}>
                  ...
                  <div key={mediaItem.url} className="carousel-item relative">
                    <Link to={`/posts/${post.id}`} className="block">
                      {mediaItem.type === 'video' ? (
                        ...
                        >
                          {mediaItem.captionsUrl && (
                            <track
                              src={mediaItem.captionsUrl}
                              kind="captions"
                              srcLang="en"
                              label="English"
                              default
              ...
              <Link to={`/posts/${post.id}`} className="block">
                {post.media[0].type === 'video' ? (
                  <video
                    ...
                    className="h-auto max-h-96 w-full rounded-2xl border border-dark-200 object-cover dark:border-dark-700"
                  >
                    {post.media[0].captionsUrl && (
                      <track
                        src={post.media[0].captionsUrl}
                        kind="captions"
                        srcLang="en"
                        label="English"
                        default
      ...
      {showComments && (
        <div className="mt-md border-t border-dark-200 pt-md dark:border-dark-700">
          {/* Recursive Comment Renderer Function */}
          {(() => {
                ...
                <div key={comment.id} className={`mb-4 ${marginLeft}`}>
                  <div className={`rounded-xl border border-dark-200 bg-dark-50 p-md dark:border-dark-700 dark:bg-dark-800/70 ${comment.isPinned ? 'ring-2 ring-blue-500' : ''} ${comment.isLocked ? 'opacity-75' : ''}`}>
                    ...
                    <div className="mt-2 flex items-center gap-4">
                      {/* Reply button - only show if comment is not locked */}
                    ...
                    <div key={comment.id} className={`mb-4 ${marginLeft}`}>
                      <div className={`rounded-xl border border-dark-200 bg-dark-50 p-md dark:border-dark-700 dark:bg-dark-800/70 ${comment.isPinned ? 'ring-2 ring-blue-500' : ''} ${comment.isLocked ? 'opacity-75' : ''}`}>
                        ...
                        {!comment.isLocked && currentUser && (
                          <button
                            onClick={() => setReplyingTo(comment.id)}
                            className="mt-2 flex items-center gap-1 text-sm text-dark-500 hover:text-blue-600 dark:hover:text-blue-400"
                          >
                            <Reply size={14} /> Reply
                          </button>
                        )}
    ...
    const isLocked = lockedComments.has(comment.id);
    const canInteract = !isLocked && currentUser;
    const isOwner = currentUser?.id === post.user.id; // Post owner can moderate all comments
    
    return (
      <div key={comment.id} style={{ marginLeft: `${depth * 24}px` }} className="rounded-xl border border-dark-200 bg-dark-50 p-md dark:border-dark-700 dark:bg-dark-800/70">
        <div className="flex items-start justify-between">
          <Link to={`/users/${comment.user.id}`} className="flex items-center gap-sm mb-xs hover:opacity-80">
            <div className="w-8 h-8 bg-gradient-to-br from-primary-300 to-accent-300 rounded-full flex items-center justify-center text-white text-xs font-bold">
              {(comment.user.email || comment.user.walletAddress || 'A').charAt(0).toUpperCase()}
            ...
            <div>
              <div className="flex items-center gap-2">
                <p className="font-semibold text-sm text-dark-900 dark:text-white">
                  {comment.user.email || comment.user.walletAddress}
                </p>
                {comment.isPinned && (
          ...
          {/* Comment Actions - only show for post owners or comment owners */}
          {(isOwner || currentUser?.id === comment.user.id) && (
            <div className="flex items-center gap-1">
              {isOwner && ( // Only post owner can pin/lock comments
                <>
                  <button
                    onClick={() => handlePinComment(comment.id)}
                    ...
                    title={comment.isPinned ? "Unpin comment" : "Pin comment"}
                  >
                    <Pin size={14} className={comment.isPinned ? "text-yellow-500" : "text-gray-500"} />
                  </button>
                  <button
                    onClick={() => handleLockComment(comment.id)}
                    className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
                  ...
                  >
                    {isLocked ? <Unlock size={14} className="text-green-500" /> : <Lock size={14} className="text-gray-500" />}
                  </button>
                </>
              )}
              {(currentUser?.id === comment.user.id || isOwner) && ( // Comment owner or post owner can delete
                <button
                  onClick={() => handleDeleteComment(comment.id)}
        ...
        <p className="text-dark-800 text-sm mt-xs dark:text-dark-200">
          {renderContent(comment.content)}
        </p>
        
        {/* Reply button - only show if comment is not locked and user is logged in */}
        {canInteract && (
          <button
            onClick={() => setReplyingTo(comment.id)}
            className="mt-2 flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 dark:text-blue-400"
          >
            <Reply size={12} /> Reply
          </button>
        )}
