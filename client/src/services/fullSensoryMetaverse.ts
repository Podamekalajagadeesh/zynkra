...
export class FullSensoryMetaverseService {
  ...
  private currentEnvironment: FullSensoryEnvironmentConfig | null = null;
    ...
    if (updates.hapticModifications) {
      updates.hapticModifications.forEach(mod => {
        ...
        if (hapticIndex !== -1) {
          ...
          if (mod.newIntensity !== undefined) {
            updatedEnvironment.ambientHaptics[hapticIndex].index = Math.max(0, Math.min(1, mod.newIntensity));
          }
        }
      });
    }
