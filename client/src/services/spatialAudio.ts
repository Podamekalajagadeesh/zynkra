...
const DEFAULT_CONFIG: SpatialAudioConfig = {
  ...
  listenerOrientation: { x: 0, y: 0, -1 }, // Looking forward along negative Z axis
  maxDistance: 100,
  rolloffFactor: 1,
  referenceDistance: 1,
  coneInnerAngle: 360,
...
export class SpatialAudioService {
  ...
  private setupParticipantTracking() {
    ...
    this.room.on('participant-connected', (participant: RemoteParticipant) => {
      this.handleNewParticipant(participant);
    });

    // Handle participants disconnecting
    this.room.on('participant-disconnected', (participant: RemoteParticipant) => {
  ...
  // Update listener (local user) position in 3D space
  ...
  // Get all participants with their positions (public method)
  getAllParticipantPositions(): Map<string, Vector3D> {
    const positions = new Map<string, Vector3D>();
    this.participantPositions.forEach((value, key) => {
      positions.set(key, { ...value.position });
...
export const spatialAudioService = new SpatialAudioService();
  ...
  // Arrange participants in a circle around the listener
  arrangeParticipantsInCircle(radius: number = 10) {
    const participants = Array.from(this.participantPositions.keys());
    const angleStep = (Math.PI * 2) / participants.length;
    ...
    participants.forEach((participantId, index) => {
      ...
      this.animateParticipantToPosition(participantId, position, 1500);
    });
    
    this.notifyListeners();
  }

  // Arrange participants in theater-style rows
  arrangeParticipantsInTheater(rowDistance: number = 5, seatsPerRow: number = 10) {
    const participants = Array.from(this.participantPositions.keys());
    
    participants.forEach((participantId, index) => {
      const row = Math.floor(index / seatsPerRow);
      const seatInRow = index % seatsPerRow;
      const seatOffset = (seatInRow - (seatsPerRow - 1) / 2) * 2; // 2 units between seats
      
      const position: Vector3D = {
        x: seatOffset,
        y: 1.7,
        z: -10 - (row * rowDistance), // Place rows behind listener (negative Z)
      };
