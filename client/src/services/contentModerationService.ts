...
// Get all active community moderation votes
export async function getActiveCommunityVotes(): Promise<CommunityVote[]> {
  const response = await fetch(`${API_BASE_URL}/moderation/community-votes/active`, {
    credentials: 'include',
  });

  if (!response.ok) {
    throw new Error('Failed to fetch active community votes');
  }

  return response.json();
}

// Start a new community vote on a moderation item
export async function startCommunityVote(queueItemId: string, voteDurationHours: number = 48): Promise<CommunityVote> {
  const response = await fetch(`${API_BASE_URL}/moderation/community-votes/start`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (!response.ok) {
    throw new Error('Failed to start community vote');
  }

  return response.json();
}

// Submit a vote in an active community moderation vote
export async function submitCommunityVote(voteId: string, vote: 'approve' | 'remove'): Promise<VoteAction> {
  const response = await fetch(`${API_BASE_URL}/moderation/community-votes/${voteId}/vote`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
// Get completed community vote results
export async function getCommunityVoteResults(): Promise<CommunityVote[]> {
  const response = await fetch(`${API_BASE_URL}/moderation/community-votes/results`, {
    credentials: 'include',
  });

  if (!response.ok) {
    throw new Error('Failed to fetch community vote results');
  }
