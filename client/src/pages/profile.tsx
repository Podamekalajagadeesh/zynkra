...
import { Copy, CheckCircle2, Wallet, UserRound, Loader2, Star, Shield, Lock, BadgeCheck } from 'lucide-react';
import { useToast } from '../hooks/useToast';
import { Skeleton } from '../components/ui/skeleton';
import { Button } from '../components/ui/button';
import { Switch } from '../components/ui/switch';
import { getProfile, getUserProfile, linkWallet, setNftPfp as apiSetNftPfp, getNfts as apiGetNfts, getReputation, followUser, unfollowUser, getMutualFollows, updatePrivacy, removeFollower, getFollowers } from '../lib/api';
...
interface UserProfile {
  ...
  following: { id: string }[];
  followers: { id: string; displayName: string; username: string; nftPfpUrl: string | null }[];
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (userId) {
          const isUserFollowing = profile.followers.some((follower: any) => follower.id === currentUser?.id);
...
setCurrentUser(updatedUser);
  ...
  const customStyles = {
    ...
    '--primary': user?.profileThemeColor || selectedTheme.colors['--primary'],
  };
