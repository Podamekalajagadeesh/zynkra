...
export class ReputationService {
  ...
  private getPointsForEvent(event: ReputationEvent): number {
    switch (event) {
      ...
      case ReputationEvent.VOTE_CAST:
        return 2;
      case ReputationEvent.DATA_SHARED:
        return 1; // 1 token for sharing data
      case ReputationEvent.INTERACTION_RECEIVED:
        return 0.5; // 0.5 tokens for each interaction received
      default:
        return 0;
    }
  }
  ...
  async addReputation(event: ReputationEvent, user: User): Promise<void> {
    ...
    const log = this.reputationLogRepository.create({
      ...
      event,
      points,
      cryptocurrencyEarned: cryptoEarned
    });
