...
export class AnalyticsService {
  ...
  async getAnalytics(userId: string) {
    ...
    // Post engagement calculations - calculate total likes using postReactions
    let totalLikes = 0;
    const postsWithReactions = await this.postsRepository.find({
      where: { user: { id: userId } },
      relations: ['postReactions'],
    });
    totalLikes = postsWithReactions.reduce((acc, post) => acc + post.postReactions.length, 0);
    ...
    const subscriptions = await this.subscriptionsRepository?.find({ where: { creatorId: userId } }) || [];
    const totalSubscriptions = subscriptions.reduce((acc, sub) => acc + Number(sub.amount), 0);
