...
export class AdsManagerService {
  // Campaign methods
  async getCampaigns(_userId: string): Promise<CampaignWithMetrics[]> {
    return campaigns;
  }
  ...
  async createCampaign(profileId: string, name: string, objective: string, adSetsData: any[] = [], _postId?: string): Promise<CampaignWithMetrics> {
  ...
  async trackImpression(adId: string, _userId: string): Promise<void> {
  ...
  async trackClick(adId: string, _userId: string): Promise<void> {
    const ad = ads.find(a => a.id === adId);
    if (ad) {
      ad.clicks++;
      const adSet = adSets.find(a => a.id === ad.ad_set_id);
  ...
  async trackConversion(adId: string, _value?: number): Promise<void> {
    const ad = ads.find(a => a.id === adId);
    if (ad) {
      ad.conversions++;
    }
