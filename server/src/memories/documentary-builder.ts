export interface DocumentaryMemoryInput {
  content?: string;
  authorName?: string;
  memoryMetadata?: Record<string, any>;
}

export interface DocumentaryBuildResult {
  title: string;
  eventName: string;
  summary: string;
...
export function buildDocumentarySummary(payload: {
  title?: string;
  eventName?: string;
  memories?: DocumentaryMemoryInput[];
  participantNames?: string[];
}): DocumentaryBuildResult {
